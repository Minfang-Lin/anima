...
每一集用几个小场景讲一种常见病在身体里是怎样发生的，幕数按内容多少来定。纯 HTML + Canvas，没有构建步骤。
...
| `sleep-drive/` | 睡意从哪来：腺苷、咖啡因、褪黑素与午睡 |
...
1. 复制一个现有主题的文件夹，改 `scene.js` 里的 `meta`（标题、`organs`、`categories`、颜色）、`CH` 和画面代码。幕数不固定，`CH` 里写几幕就是几幕，页头和视频片头会自动显示“N 幕”，改 `index.html` 里的 `data-episode` 和 `video-meta`。
...
| # | 场景 | 画面重点 | 关键数据 |
|---|------|----------|----------|
| 1 | 骨头是活的 | 骨小梁网架；紫色破骨细胞啃骨头，蓝色成骨细胞排队砌新骨 | 每年约 10% 骨骼更新 |
| 2 | 骨量像存款 | 骨量随年龄变化的示意曲线（男/女），绝经后女性下降更快 | 约 30 岁达到峰值骨量 |
| 3 | 骨量减少 | 骨小梁变细 | T 值 -1 到 -2.5 |
| 4 | 骨质疏松 | 骨小梁断开、孔洞变大，骨髓脂肪变多 | T ≤ -2.5；50 岁以上患病率约 19%，女性约 32% |
| 5 | 一摔就骨折 | 骨小梁裂纹；脊椎卡片里一节椎体被压成楔形 | 身高矮 4 cm 以上警惕椎体骨折 |
| 6 | 保护骨头 | 钙、维生素 D、成骨细胞加油，破骨细胞吃药后打瞌睡 | 50 岁以上每天钙 1000–1200 mg |
...
> **推荐做法：画面用脚本录，配音和音乐在剪映里做。** 一步一步的教程见 [docs/本地做视频（剪映配音）.md](docs/本地做视频（剪映配音）.md)：`node export_script.js <主题>` 导出配音稿，剪映朗读后按每段时长运行 `node record.js <主题> --intro 5 --durs 28,25,...` 录出对得上的画面。
...
`make_audio.py` 会用离线的 Kokoro 中文语音模型把每一幕的旁白读出来（开头加一句引子），再用代码合成一段轻柔的八音盒背景音乐（没有版权问题），混成一条音轨。每一幕的时长会跟着配音长度自动调整，一集大约 2 分半。
